...
int updateCodeWordByType(
    ...
    if (addressType == IMMEDIATE) {
        ...
        if(!currentSymbol) {
            printLineError(codeInstructionsList[instructionIndex].lineIndex, "The symbol: %s does not exist in the symbol table", symbolName);
            return FALSE;
        }
        codeInstructionsList[instructionIndex+*indexInLine].base = currentSymbol->base;
...
void validateMachineCodeLimitation(int IC, int DC, int lineIndex) {
	if (IC - IC_INIT_VALUE + DC > MAX_MACHINE_CODE_SECTION){
		printLineError(lineIndex, "The machine code can only include %d words", MAX_MACHINE_CODE_SECTION);
        exit(EXIT_FAILURE);
    }
}
